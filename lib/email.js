...
export async function sendQuoteEmail(to, customerName, quote, pdfUrl, quoteUrl) {
  ...
  const itemsHtml = itemsList.map((item, i) => `
    <tr style="background:${i % 2 === 0 ? '#f9f9f9' : '#fff'};">
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:center;color:#555;">${item.qty || 1}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;color:#555;">₹${parseFloat(item.price || 0).toFixed(2)}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;color:#333;font-weight:bold;">₹${((parseFloat(item.price || 0)) * (parseInt(item.qty || 1))).toFixed(2)}</td>
    </tr>
  `).join('');
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
                      <thead>
                        <tr style="background:#6C63FF;">
                          ...
                          <th style="padding:12px 8px;text-align:right;color:#fff;font-size:10px;text-transform:uppercase;">Rate</th>
