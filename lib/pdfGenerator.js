...
export async function generateInvoicePDF(invoice, ticket, items, excludingGst = false) {
    ...
    // Colors
    const primaryColor = [0, 59, 115]; // #003B73
    ...
    doc.text(`Ph: ${COMPANY_INFO.phone}`, companyRightX, 28);
    doc.text(COMPANY_INFO.email, companyRightX, 33);
    doc.text(`GSTIN: ${COMPANY_INFO.gstin}`, companyRightX, 40);
    doc.text(`CIN: ${COMPANY_INFO.cin}`, companyRightX, 46);
    ...
    doc.setTextColor(...primaryColor);
    doc.setFont('helvetica', 'bold');
    doc.text('ESTIMATE', pageWidth / 2, 62, { align: 'center' });
    ...
    autoTable(doc, {
        startY: 105,
        head: [['S.No', 'Description', 'Qty', 'Unit', 'MRP', 'Selling Price', 'Amount']],
        body: tableData,
        theme: 'striped',
        headStyles: {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('For, KZ COMPUTERS', pageWidth - 15, 275, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
...
export function generateInvoicePDFBuffer(invoice, ticket, items, excludingGst = false) {
    ...
    doc.setDrawColor(...primaryColor);
    doc.setLineWidth(0.3);
    doc.line(15, 277, pageWidth - 15, 277);

    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text('For, KZ COMPUTERS', pageWidth - 15, 287, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.text('Authorized Signatory', pageWidth - 15, 294, { align: 'right' });

    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text(`${COMPANY_INFO.address} | GSTIN: ${COMPANY_INFO.gstin} | CIN: ${COMPANY_INFO.cin}`, pageWidth / 2, 304, { align: 'center' });
    doc.text(`Ph: ${COMPANY_INFO.phone} | Email: ${COMPANY_INFO.email}`, pageWidth / 2, 309, { align: 'center' });
...
export function generateQuotePDFBuffer(quote, customer, items, message) {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    ...
    doc.text(`GSTIN: ${COMPANY_INFO.gstin} | CIN: ${COMPANY_INFO.cin}`, 15, 76);
    ...
    doc.text(`${COMPANY_INFO.name} | ${COMPANY_INFO.address} | GSTIN: ${COMPANY_INFO.gstin} | CIN: ${COMPANY_INFO.cin}`, pageWidth / 2, 285, { align: 'center' });
    doc.text(`Ph: ${COMPANY_INFO.phone} | Email: ${COMPANY_INFO.email}`, pageWidth / 2, 290, { align: 'center' });
